...
| Fichier | Git | Rôle |
|---------|-----|------|
| `settings.json` | oui | Config publique **dev** (port **3001**, host `127.0.0.1`…) |
| `settings.prod.example.json` | oui | Modèle config **prod** (port **3102**, `env: production`) |
...
Exemple : https://ams.dynatech.app — nginx reverse-proxy vers Node en local (`127.0.0.1:3102`).

1. Copier / adapter la config publique (ex. s’inspirer de `settings.prod.example.json` : `"port": 3102`, `"host": "127.0.0.1"`, `"env": "production"`).
...
Le navigateur doit appeler `/api/...` sur le même host HTTPS (pas `http://127.0.0.1:3001`).
...
├── settings.json        # Config publique dev (port 3001)
├── settings.prod.example.json
